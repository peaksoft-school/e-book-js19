...
import { LogoutModal } from '../../../shared/ui/modal/LogoutModal';
...
export const Header = ({
  ...
  const ProfileDropdown = ({ withProfile = false }: { withProfile?: boolean }) => (
    <div className="absolute right-0 top-10 bg-white border border-neutral-200 shadow-md z-50 min-w-40">
      {withProfile && (
        <button
          type="button"
          onClick={() => {
          ...
          className="w-full text-left px-4 py-2.5 text-body text-primary hover:bg-neutral-100 transition-colors"
        >
          Профиль
        </button>
      )}
      <button
        ...
        className="w-full text-left px-4 py-2.5 text-body text-danger hover:bg-neutral-100 transition-colors"
      >
        Выйти
      </button>
    </div>
  ...
  return (
    <>
      <header className={`bg-white w-full ${variant === 'admin' ? 'px-0' : 'px-20'}`}>
            ...
            {variant === 'vendor' && (
              <>
                ...
                <div className="relative">
                  ...
                  {showProfileMenu && <ProfileDropdown withProfile />}
                </div>
              </>
            )}
