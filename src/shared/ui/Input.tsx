...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className, variant, label, required, placeholder, ...props }, ref) => {
    ...
    const computedPlaceholder = required && placeholder ? `${placeholder} *` : placeholder;

    return (
      <div className="flex flex-col gap-1 w-full">
        {label && <label className="text-body text-primary">{label}</label>}
        <div
          className={cn(
            'relative flex items-center border transition-colors',
            isFocused ? 'border-secondary' : 'border-neutral-200'
          )}
        >
          <input
            ref={ref}
